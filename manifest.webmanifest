...
async function fetchTrips() {
  const result = await listRows(CONFIG.tables.trips, 500);
...
async function fetchAllParticipants() {
  const result = await listRows(CONFIG.tables.participants, 500);
...
async function fetchExpenses(tripId) {
  const result = await listRows(CONFIG.tables.expenses, 500);
  ...
  return { expenses, meta, tripMetaMap };
}
...
async function loadSharedData({ silent = false, force = false } = {}) {
  ...
  try {
    ...
    if (isSharedTripMode() && !inviteTripId) {
      inviteTripId = await Promise.race([
...
function renderHeader() {
  ...
  $("#topbarEyebrow").textContent = isSharedTripMode()
    ? "모아트립 · 초대받은 여행"
    : "모아트립 · 함께 쓰는 여행 장부";

  $("#headerTripName").textContent = state.trip.name || "여행 가계부";
  $("#tripSwitcherButton").disabled = isSharedTripMode();
  $("#quickEditTripTitleBtn").disabled = isSharedTripMode() || isLedgerLocked();
...
function renderTripSwitcher() {
  ...
  list.innerHTML = state.trips.length
    ? state.trips.map((trip) => {
        ...
        return `
          <button type="button" class="trip-switch-card${active ? " active" : ""}" data-switch-trip="${escapeHtml(trip.id)}">
    ...
    : '<div class="empty-state compact"><strong>아직 만든 여행이 없어.</strong></div>';
}
...
function renderSettingsValues() {
  ...
  ["#tripNameInput", "#tripLocationInput", "#tripStartInput", "#tripEndInput", "#newParticipantInput", "#addParticipantBtn", "#saveTripSettingsBtn"]
    .forEach((selector) => {
...
async function ensureShareToken({ renew = false } = {}) {
  if (isSharedTripMode() && INVITE_TOKEN && !renew) {
    return INVITE_TOKEN;
  }

  const currentToken = String(state.meta?.shareToken || "");
  if (currentToken && !renew) return currentToken;

  const token = generateShareToken();
  await saveSharedMeta({ shareToken: token });
  return token;
}
...
async function copyShareLink() {
  ...
  try {
    ...
    showToast("초대 링크를 복사했어.");
  } catch {
    prompt("아래 링크를 복사해줘.", url);
  }
}
  ...
  if (navigator.share) {
    try {
      await navigator.share({
        title,
        text: `${title}${locationText} 모아트립 장부 같이 쓰자!`,
        url,
      });
      return;
    } catch (error) {
...
async function renewShareLink() {
  if (!confirm("기존 초대 링크는 더 이상 사용할 수 없게 돼. 새로 만들까?")) {
    return;
  }
  ...
  try {
    const token = await ensureShareToken({ renew: true });
    $("#shareLinkInput").value = buildInviteUrl(token);
    showToast("새 초대 링크를 만들었어.");
  } catch (error) {
    alert(readableError(error));
  } finally {
    hideLoading();
